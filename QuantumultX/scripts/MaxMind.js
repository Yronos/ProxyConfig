// MaxMind GeoIP2 for Quantumult X
// $response.statusCode, $response.headers, $response.body

// 1. 状态码检查
if ($response.statusCode != 200) {
  $done(null);
}

// 2. 默认值配置
const DEFAULTS = {
  location: "未知位置",
  isp: "未知 ISP",
  timezone: "未知时区",
  flag: "🏳️",
};

// 3. 下标数字映射
const SUBSCRIPT_MAP = {
  0: "₀",
  1: "₁",
  2: "₂",
  3: "₃",
  4: "₄",
  5: "₅",
  6: "₆",
  7: "₇",
  8: "₈",
  9: "₉",
  ".": ".",
};

// 4. 国旗映射表
const flags = new Map([
  ["AC", "🇦🇨"],
  ["AD", "🇦🇩"],
  ["AE", "🇦🇪"],
  ["AF", "🇦🇫"],
  ["AG", "🇦🇬"],
  ["AI", "🇦🇮"],
  ["AL", "🇦🇱"],
  ["AM", "🇦🇲"],
  ["AO", "🇦🇴"],
  ["AQ", "🇦🇶"],
  ["AR", "🇦🇷"],
  ["AS", "🇦🇸"],
  ["AT", "🇦🇹"],
  ["AU", "🇦🇺"],
  ["AW", "🇦🇼"],
  ["AX", "🇦🇽"],
  ["AZ", "🇦🇿"],
  ["BA", "🇧🇦"],
  ["BB", "🇧🇧"],
  ["BD", "🇧🇩"],
  ["BE", "🇧🇪"],
  ["BF", "🇧🇫"],
  ["BG", "🇧🇬"],
  ["BH", "🇧🇭"],
  ["BI", "🇧🇮"],
  ["BJ", "🇧🇯"],
  ["BL", "🇧🇱"],
  ["BM", "🇧🇲"],
  ["BN", "🇧🇳"],
  ["BO", "🇧🇴"],
  ["BQ", "🇧🇶"],
  ["BR", "🇧🇷"],
  ["BS", "🇧🇸"],
  ["BT", "🇧🇹"],
  ["BV", "🇧🇻"],
  ["BW", "🇧🇼"],
  ["BY", "🇧🇾"],
  ["BZ", "🇧🇿"],
  ["CA", "🇨🇦"],
  ["CC", "🇨🇨"],
  ["CD", "🇨🇩"],
  ["CF", "🇨🇫"],
  ["CG", "🇨🇬"],
  ["CH", "🇨🇭"],
  ["CI", "🇨🇮"],
  ["CK", "🇨🇰"],
  ["CL", "🇨🇱"],
  ["CM", "🇨🇲"],
  ["CN", "🇨🇳"],
  ["CO", "🇨🇴"],
  ["CP", "🇨🇵"],
  ["CR", "🇨🇷"],
  ["CU", "🇨🇺"],
  ["CV", "🇨🇻"],
  ["CW", "🇨🇼"],
  ["CX", "🇨🇽"],
  ["CY", "🇨🇾"],
  ["CZ", "🇨🇿"],
  ["DE", "🇩🇪"],
  ["DG", "🇩🇬"],
  ["DJ", "🇩🇯"],
  ["DK", "🇩🇰"],
  ["DM", "🇩🇲"],
  ["DO", "🇩🇴"],
  ["DZ", "🇩🇿"],
  ["EA", "🇪🇦"],
  ["EC", "🇪🇨"],
  ["EE", "🇪🇪"],
  ["EG", "🇪🇬"],
  ["EH", "🇪🇭"],
  ["ER", "🇪🇷"],
  ["ES", "🇪🇸"],
  ["ET", "🇪🇹"],
  ["EU", "🇪🇺"],
  ["FI", "🇫🇮"],
  ["FJ", "🇫🇯"],
  ["FK", "🇫🇰"],
  ["FM", "🇫🇲"],
  ["FO", "🇫🇴"],
  ["FR", "🇫🇷"],
  ["GA", "🇬🇦"],
  ["GB", "🇬🇧"],
  ["GD", "🇬🇩"],
  ["GE", "🇬🇪"],
  ["GF", "🇬🇫"],
  ["GG", "🇬🇬"],
  ["GH", "🇬🇭"],
  ["GI", "🇬🇮"],
  ["GL", "🇬🇱"],
  ["GM", "🇬🇲"],
  ["GN", "🇬🇳"],
  ["GP", "🇬🇵"],
  ["GQ", "🇬🇶"],
  ["GR", "🇬🇷"],
  ["GS", "🇬🇸"],
  ["GT", "🇬🇹"],
  ["GU", "🇬🇺"],
  ["GW", "🇬🇼"],
  ["GY", "🇬🇾"],
  ["HK", "🇭🇰"],
  ["HM", "🇭🇲"],
  ["HN", "🇭🇳"],
  ["HR", "🇭🇷"],
  ["HT", "🇭🇹"],
  ["HU", "🇭🇺"],
  ["IC", "🇮🇨"],
  ["ID", "🇮🇩"],
  ["IE", "🇮🇪"],
  ["IL", "🇮🇱"],
  ["IM", "🇮🇲"],
  ["IN", "🇮🇳"],
  ["IO", "🇮🇴"],
  ["IQ", "🇮🇶"],
  ["IR", "🇮🇷"],
  ["IS", "🇮🇸"],
  ["IT", "🇮🇹"],
  ["JE", "🇯🇪"],
  ["JM", "🇯🇲"],
  ["JO", "🇯🇴"],
  ["JP", "🇯🇵"],
  ["KE", "🇰🇪"],
  ["KG", "🇰🇬"],
  ["KH", "🇰🇭"],
  ["KI", "🇰🇮"],
  ["KM", "🇰🇲"],
  ["KN", "🇰🇳"],
  ["KP", "🇰🇵"],
  ["KR", "🇰🇷"],
  ["KW", "🇰🇼"],
  ["KY", "🇰🇾"],
  ["KZ", "🇰🇿"],
  ["LA", "🇱🇦"],
  ["LB", "🇱🇧"],
  ["LC", "🇱🇨"],
  ["LI", "🇱🇮"],
  ["LK", "🇱🇰"],
  ["LR", "🇱🇷"],
  ["LS", "🇱🇸"],
  ["LT", "🇱🇹"],
  ["LU", "🇱🇺"],
  ["LV", "🇱🇻"],
  ["LY", "🇱🇾"],
  ["MA", "🇲🇦"],
  ["MC", "🇲🇨"],
  ["MD", "🇲🇩"],
  ["ME", "🇲🇪"],
  ["MF", "🇲🇫"],
  ["MG", "🇲🇬"],
  ["MH", "🇲🇭"],
  ["MK", "🇲🇰"],
  ["ML", "🇲🇱"],
  ["MM", "🇲🇲"],
  ["MN", "🇲🇳"],
  ["MO", "🇲🇴"],
  ["MP", "🇲🇵"],
  ["MQ", "🇲🇶"],
  ["MR", "🇲🇷"],
  ["MS", "🇲🇸"],
  ["MT", "🇲🇹"],
  ["MU", "🇲🇺"],
  ["MV", "🇲🇻"],
  ["MW", "🇲🇼"],
  ["MX", "🇲🇽"],
  ["MY", "🇲🇾"],
  ["MZ", "🇲🇿"],
  ["NA", "🇳🇦"],
  ["NC", "🇳🇨"],
  ["NE", "🇳🇪"],
  ["NF", "🇳🇫"],
  ["NG", "🇳🇬"],
  ["NI", "🇳🇮"],
  ["NL", "🇳🇱"],
  ["NO", "🇳🇴"],
  ["NP", "🇳🇵"],
  ["NR", "🇳🇷"],
  ["NU", "🇳🇺"],
  ["NZ", "🇳🇿"],
  ["OM", "🇴🇲"],
  ["PA", "🇵🇦"],
  ["PE", "🇵🇪"],
  ["PF", "🇵🇫"],
  ["PG", "🇵🇬"],
  ["PH", "🇵🇭"],
  ["PK", "🇵🇰"],
  ["PL", "🇵🇱"],
  ["PM", "🇵🇲"],
  ["PN", "🇵🇳"],
  ["PR", "🇵🇷"],
  ["PS", "🇵🇸"],
  ["PT", "🇵🇹"],
  ["PW", "🇵🇼"],
  ["PY", "🇵🇾"],
  ["QA", "🇶🇦"],
  ["RE", "🇷🇪"],
  ["RO", "🇷🇴"],
  ["RS", "🇷🇸"],
  ["RU", "🇷🇺"],
  ["RW", "🇷🇼"],
  ["SA", "🇸🇦"],
  ["SB", "🇸🇧"],
  ["SC", "🇸🇨"],
  ["SD", "🇸🇩"],
  ["SE", "🇸🇪"],
  ["SG", "🇸🇬"],
  ["SH", "🇸🇭"],
  ["SI", "🇸🇮"],
  ["SJ", "🇸🇯"],
  ["SK", "🇸🇰"],
  ["SL", "🇸🇱"],
  ["SM", "🇸🇲"],
  ["SN", "🇸🇳"],
  ["SO", "🇸🇴"],
  ["SR", "🇸🇷"],
  ["SS", "🇸🇸"],
  ["ST", "🇸🇹"],
  ["SV", "🇸🇻"],
  ["SX", "🇸🇽"],
  ["SY", "🇸🇾"],
  ["SZ", "🇸🇿"],
  ["TA", "🇹🇦"],
  ["TC", "🇹🇨"],
  ["TD", "🇹🇩"],
  ["TF", "🇹🇫"],
  ["TG", "🇹🇬"],
  ["TH", "🇹🇭"],
  ["TJ", "🇹🇯"],
  ["TK", "🇹🇰"],
  ["TL", "🇹🇱"],
  ["TM", "🇹🇲"],
  ["TN", "🇹🇳"],
  ["TO", "🇹🇴"],
  ["TR", "🇹🇷"],
  ["TT", "🇹🇹"],
  ["TV", "🇹🇻"],
  ["TW", "🇨🇳"],
  ["TZ", "🇹🇿"],
  ["UA", "🇺🇦"],
  ["UG", "🇺🇬"],
  ["UK", "🇬🇧"],
  ["UM", "🇺🇲"],
  ["US", "🇺🇸"],
  ["UY", "🇺🇾"],
  ["UZ", "🇺🇿"],
  ["VA", "🇻🇦"],
  ["VC", "🇻🇨"],
  ["VE", "🇻🇪"],
  ["VG", "🇻🇬"],
  ["VI", "🇻🇮"],
  ["VN", "🇻🇳"],
  ["VU", "🇻🇺"],
  ["WF", "🇼🇫"],
  ["WS", "🇼🇸"],
  ["XK", "🇽🇰"],
  ["YE", "🇾🇪"],
  ["YT", "🇾🇹"],
  ["ZA", "🇿🇦"],
  ["ZM", "🇿🇲"],
  ["ZW", "🇿🇼"],
]);

// 5. 工具函数：IP 格式化（IPv4 转下标，IPv6 保持原样）
function formatIP(ip) {
  if (!ip) return "N/A";
  // IPv6 直接返回
  if (ip.indexOf(":") !== -1) {
    return ip;
  }
  // IPv4 转下标
  return ip
    .toString()
    .split("")
    .map((c) => SUBSCRIPT_MAP[c] || c)
    .join("");
}

// 6. 工具函数：从多语言 names 对象获取名称
function getName(namesObj) {
  if (!namesObj) return null;
  if (namesObj["zh-CN"]) return namesObj["zh-CN"].trim();
  if (namesObj["en"]) return namesObj["en"].trim();
  const keys = Object.keys(namesObj);
  if (keys.length > 0 && namesObj[keys[0]]) {
    return String(namesObj[keys[0]]).trim();
  }
  return null;
}

// 7. 解析响应数据
var body = $response.body;
var obj = JSON.parse(body);

// 8. 提取 IP 地址
var ip = (obj.traits && obj.traits.ip_address) || "N/A";

// 9. 提取国家代码和国旗
var countryCode = (obj.country && obj.country.iso_code) || "";
var flag = flags.get(countryCode) || DEFAULTS.flag;

// 10. 位置降级：城市 -> 州/省 -> 国家
var city = obj.city && obj.city.names ? getName(obj.city.names) : null;
var region =
  obj.subdivisions && obj.subdivisions[0] && obj.subdivisions[0].names
    ? getName(obj.subdivisions[0].names)
    : null;
var country =
  obj.country && obj.country.names ? getName(obj.country.names) : null;
var location = city || region || country || DEFAULTS.location;

// 11. ISP 降级：ISP -> Organization -> ASN Org -> Network
var isp = null;
var network = null;
var asn = null;

if (obj.traits) {
  isp =
    obj.traits.isp ||
    obj.traits.organization ||
    obj.traits.autonomous_system_organization ||
    null;
  network = obj.traits.network || null;
  asn = obj.traits.autonomous_system_number
    ? "AS" + obj.traits.autonomous_system_number
    : null;
}

// 12. 时区
var timezone = (obj.location && obj.location.time_zone) || DEFAULTS.timezone;

// 13. 构建输出
var title = flag + " " + location + " " + formatIP(ip);

var subtitleParts = [];
if (isp) {
  subtitleParts.push(asn ? isp + " (" + asn + ")" : isp);
} else if (asn) {
  subtitleParts.push(asn);
} else if (network) {
  subtitleParts.push(network);
}
subtitleParts.push(timezone);

var subtitle = subtitleParts.join(" | ");

// 14. 构建详细描述
var descParts = [];
if (country) descParts.push("国家: " + country);
if (region && region !== city) descParts.push("地区: " + region);
if (city) descParts.push("城市: " + city);
if (isp) descParts.push("ISP: " + isp);
if (asn) descParts.push("ASN: " + asn);
if (network) descParts.push("网段: " + network);
descParts.push("时区: " + timezone);

var description = descParts.join("\n");

// 15. 返回结果
$done({ title, subtitle, ip, description });
