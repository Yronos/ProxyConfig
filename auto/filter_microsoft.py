...
OUTPUT_FILENAME = "Microsoft.list"

# 主规则源：你要精简的 Microsoft 规则
MAIN_RULESET = {
    "name": "Sukka Microsoft",
    "url": "https://raw.githubusercontent.com/SukkaLab/ruleset.skk.moe/refs/heads/master/List/non_ip/microsoft.conf",
}

# 用于从 Microsoft 中剔除的规则集
# 例如你有单独的 Google / Twitter 策略组，那么这些规则就没必要再留在 Microsoft 里
EXCLUDE_RULESETS = []
...
# True  = 手动添加优先，强制保留
# False = 只要命中排除规则集，就不加入 Microsoft
...
def write_output(rules: list, stats: dict, exclude_results: list):
    ...
    if WRITE_HEADER:
        lines.extend(
            [
                "# Generated by filter_microsoft.py",
...
def main():
    try:
        rules, stats, exclude_results = generate_lite_microsoft()
