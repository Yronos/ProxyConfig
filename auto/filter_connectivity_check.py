...
"""
filter_connectivity_check.py

从主规则集中生成精简版 connectivity_check.list：
...
OUTPUT_FILENAME = "connectivity_check.list"

# 主规则源：你要精简的 connectivity_check 规则
MAIN_RULESET = {
    "name": "MetaCubeX meta-rules-dat connectivity_check",
    "url": "https://raw.githubusercontent.com/MetaCubeX/meta-rules-dat/refs/heads/meta/geo/geosite/classical/connectivity-check.list",
}

# 用于从 connectivity_check 中剔除的规则集
# 例如你有单独的 Google / Twitter 策略组，那么这些规则就没必要再留在 connectivity_check 里
EXCLUDE_RULESETS = [
]
...
CUSTOM_REMOVE_RULES = [
    ...
    "DOMAIN-SUFFIX,kuketz.de",
    "DOMAIN,www.gstatic.com",
    "DOMAIN,nmcheck.gnome.org",
    "DOMAIN-SUFFIX,network-test.debian.org",
    "DOMAIN,connect.rom.miui.com",
    "DOMAIN,conn1.oppomobile.com",
    "DOMAIN,conn2.oppomobile.com",
]

# 手动添加规则
# 这些规则会追加到最终 connectivity_check.list
CUSTOM_ADD_RULES = [
...
# True  = 手动添加优先，强制保留
# False = 只要命中排除规则集，就不加入 connectivity_check
...
def generate_lite_connectivity_check():
    """
    核心流程：
    1. 加载主 connectivity_check 规则
    2. 加载多个排除规则集
    3. 合并排除规则
    4. 过滤主 connectivity_check
    5. 手动添加规则
    6. 输出去重后的规则
    """

    print("============================================================")
    print("生成 Lite connectivity_check.list")
    print("============================================================")
...
def write_output(rules: list, stats: dict, exclude_results: list):
    ...
    if WRITE_HEADER:
        lines.extend(
            [
                "# Generated by filter_connectivity_check.py",
...
def main():
    try:
        rules, stats, exclude_results = generate_lite_connectivity_check()
